		...
		const user = member.user;
			...
			if(moment(user.createdAt).format('MM') === '12') {
				var tarih = `${moment(user.createdAt).format('DD')} Aralık ${moment(user.createdAt).format('YYYY HH:mm:ss')} `
					...
					.addField(`❯ ID:`, `${user.id}`)
					.addField(`❯ Discord a Kayıt tarihi:`, tarih)
					.addField(`❯ Durumu:`, `${durm}`)
					.addField(`❯ Şu an oynadığı oyun:`, `${user.presence.game ? user.presence.game.name : 'Aradığınız Kullanıcı Şu Anlık Oyun Oynamıyor.'}`)
					.addField(`❯ Bot mu?`, `${user.bot ? '\nEvet Bu Bir Bot!' : 'Hayır Bu Bir Kullanıcı!'}`)
