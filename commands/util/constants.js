...
exports.responses = {
  ...
  COMMANDO: {
    ARGUMENT: {
      ...
      RESPOND_WITH_CANCEL: {
        english: " \`cancel\` yazarak komudu iptal ettilebilirsiniz! ",
        klingon: "respond \`cancel\` ra' qIl.",
        spanish: "Responde con \`cancel\` para cancelar el comando."
      },
      WAIT: {
        english: (time) => ` ${time} Saniye İçerisinde Eğer Gerekli İşlemi Yapmaz iseniz komut iptal olacak.`,
        klingon: (time) => `${time} Saniye İçerisinde Eğer Gerekli İşlemi Yapmaz iseniz komut iptal olacak.`,
        spanish: (time) => `${time} Saniye İçerisinde Eğer Gerekli İşlemi Yapmaz iseniz komut iptal olacak.`
      }
    }
  },
