			...
			while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
				...
				if (!opponent.bot || (opponent.bot && userTurn)) {
					await message.say(stripIndents`
						${user}, ne yapmak istersin? **saldır**, **korun**, **efsane**, veya **pes et (niye pes edicen ki korkak mısın?**?
				...
				if (choice === 'saldır') {
					...
					await message.say(`${user}, **${damage}** HP Hasar Vurdu!!`);
					dealDamage(damage);
					reset();
				} else if (choice === 'korun') {
					await message.say(`${user}, kendini korumaya aldı!!`);
					guard = true;
					reset(false);
					...
					await message.say(`${user}, Adlı Kullanıcı Pes Etti Korkak GAHAHAH !`);
					forfeit();
					break;
				} else {
